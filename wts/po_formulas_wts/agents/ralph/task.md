...
**Paths.** Code lives in `{{pack_path}}` (`cd "${WORK_DIR:-{{pack_path}}}"` before any edit / `git add` / `git commit`). The bead + run_dir live under `{{rig_path}}` — write the verdict artifact there. When `{{pack_path}}` equals `{{rig_path}}` the two are the same directory.
{{gate_failures_block}}
