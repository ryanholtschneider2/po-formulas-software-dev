...
Run all `git` + lint-fix commands inside `{{pack_path}}` (`cd {{pack_path}}` first) — that's the repo where build commits landed. Persist the lint log to `{{run_dir}}/lint-iter-{{iter}}.log` under `{{rig_path}}`. When `{{pack_path}}` == `{{rig_path}}` they're the same directory.
