...
- `software-dev-agentic`: one prompt-driven actor opens a worktree off `main`, builds, runs the repo's own tests/CI, and opens a PR — looped against one critic that verifies goal accomplishment (`pass`/`fail`). For SoloCo product work, set `PO_AGENTIC_PROOF_MODE=strict` in the rig's `.po-env` to require fresh review artifacts and live verification even for sizing-classified low-risk work; keep `PO_PREVIEW` and `PO_DEMO_VIDEO` as separate surface-specific knobs. Never auto-merges. See README §`software-dev-agentic` and `docs/soloco-strict-product-mode.md`.
