...
- `software-dev-agentic`: one prompt-driven actor opens a worktree off `main`, builds, runs the repo's own tests/CI, and opens a PR — looped against a focused design-review critic and then the goal-accomplishment critic (`pass`/`fail`). No machine gate layer; never auto-merges. See README §`software-dev-agentic`.
