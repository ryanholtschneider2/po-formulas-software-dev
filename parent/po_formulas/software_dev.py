...
which downstream steps actually run:
  ...
  complex   — + e2e/playwright + regression-gate + deploy-smoke +
              verifier + demo-video (has_ui). Ralph + full-test-gate are
...
from prefect_orchestration.role_artifacts import publish_run_artifacts
...
def _write_artifact_contract(
    *,
    ...
    """Write the canonical handoff summary + manifest and publish them."""
    ensure_handoff_summary(
        run_dir,
        issue_id=issue_id,
    ...
    publish_run_artifacts(
        run_dir,
        [REVIEW_SUMMARY, ARTIFACT_MANIFEST],
        issue_id=issue_id,
    )
    ...
    try:
        ...
        if complexity == "trivial":
            ...
            if claim and not dry_run:
                close_issue(
                    issue_id,
                    notes=format_handoff_note(
                        "po simple-mode complete (trivial path)", run_dir
                    ),
                    rig_path=rig_path_p,
                )
            return {
        ...
        if is_docs_only:
            ...
            if claim and not dry_run:
                close_issue(
                    issue_id,
                    notes=format_handoff_note("docs-only path complete", run_dir),
                    rig_path=rig_path_p,
                )
            return {
        ...
        run_demo_video = complexity in ("moderate", "complex") and has_ui
        ...
        if claim and not dry_run:
            close_issue(
                issue_id,
                notes=format_handoff_note("po simple-mode complete", run_dir),
                rig_path=rig_path_p,
            )
...
def software_dev_edit(
    ...
    if claim and not dry_run:
        close_issue(
            issue_id,
            notes=format_handoff_note("po edit-mode complete", run_dir),
            rig_path=rig_path_p,
        )
