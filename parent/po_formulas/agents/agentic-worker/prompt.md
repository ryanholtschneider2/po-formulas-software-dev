...
4. **AUTO-FORMAT, then get the repo's local gates green BEFORE you open the PR.** First run the repo's **formatter** so format-check drift can't red CI — a linter/type-check passing does NOT imply format-clean, and that drift is the classic "green locally, red in CI" break. Prefer a single format-and-validate command if the repo ships one (**`make ready`** in Holtschneider LLC repos = auto-format then the full CI gate); otherwise run the repo's formatter (`make format`, `prettier --write .`, `ruff format .`) and THEN its lint/format + full test gates (a `make` target, the documented `pytest`/npm/bun invocation, any pre-PR smoke/e2e the rig ships) **in your worktree**, and get them green. Verifying CI passes locally first is required — never push a red branch hoping CI sorts it out, and never open/ready a PR on red or un-run gates. Commit the formatting changes too. Commit early and often as you go.
