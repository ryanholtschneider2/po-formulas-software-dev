...
2. **Are the repo's own tests / CI green — proven by a captured run, not a claim?** The actor was told to run them and tee the output. Do **not** take a stated pass/fail count on faith: confirm a **just-produced captured command output that includes the runner's own summary line and a visible exit code** (in the tee'd log, `review-artifacts/`, or pasted in the turn). A "done / 8/8 passing" with no exit-0 artifact backing that exact number is an **automatic FAIL**, not a glance-and-pass — fabricated or stale counts are the single most common way a red suite ships green. If the captured output is cheap to reproduce and you doubt it, re-run it yourself. Evidence missing, contradicting "done," or not matching the claimed count = FAIL.
...
8. **Did the named structure actually get built — not a hollow stand-in?** When the bead names a *control structure* or a concrete shape (an actor-critic loop, a max-iteration cap, retry/routing, a state machine, a debounce, a specific algorithm, "remove feature X / keep features Y and Z"), confirm that structure is present in the diff — read the code, don't trust the summary. A single-shot wrapper closed as "actor-critic loop done", a hard-coded value standing in for the requested logic, or a "passing" implementation that quietly omits the named mechanism is a **FAIL**, even if it runs. The spec, not just the build, is the bar.
9. **Did adjacent, already-working behavior survive?** The change must not silently break or delete behavior the bead didn't intend to touch. Check the surfaces neighbouring the change — for an interactive surface, confirm the actor exercised the *adjacent* flows (the controls/options/screens next to the new one), not only the new path. A feature that lands by dropping a sibling capability the user still relies on (a removed effort-level control, a vanished list, a regressed option) is a **FAIL** with a concrete note on what regressed.

# Verdict

- `pass` — the change faithfully accomplishes the goal, tests are green **and proven by a captured exit-0 run whose summary matches the claim**, rigor matches the ask, the named structure is actually present in the diff, no adjacent behavior regressed, the loop is closed in a real setting (or the deferral is explicit + tracked), it's polished to the bar (visual surfaces shown rendered), docs are current where behavior changed, and a PR is open (or the actor gave a concrete reason none could be opened). The seed closes.
- `fail` — the change does not accomplish the goal, or tests are red / the green claim isn't backed by a captured run, or required rigor is missing, or the named control structure is faked/hollow, or it broke an adjacent already-working flow, or it ships visibly unpolished / as a fake redesign, or behavior-changing docs are stale or missing. **Return a concrete, numbered fix list** so the actor can iterate. Write it to `{{run_dir}}/critique-iter-{{iter}}.md` (the flow feeds this file back to the actor on the next turn) before closing.
